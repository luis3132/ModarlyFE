...
const VentasMes: FC<VentasRangoProps> = ({ ventas }) => {
    const dias = getDaysInMonth(new Date().getFullYear(), new Date().getMonth());
    return (
        <>
                        ...
                        <div className="w-[80%] max-md:w-full rounded-xl p-5 custom-scrollbar mb-10">
                            {ventas.length === 0 ? (
                                ...
                                <table className="w-full">
                                    ...
                                    <tbody>
                                        {dias.map((dia) => {
                                            const ventasDia = ventas.filter(venta => new Date(venta.fecha).getDate() === dia);
                                            return (
                                                <>
                                                    {ventasDia.length > 0 && (
                                                        <tr className="w-full text-center">
                                                            <td colSpan={6}>Ventas {dia}</td>
                                                        </tr>
                                                    )}
